		...
		while line:
			...
			if line.find('--inputs') != -1:
				...
				if prefix == 'i' and m_input:
					inp.append(m_input.group(1).strip('_'))
					result += '''\n    %s = Bool('%s')'''%(m_input.group(1).strip('_'), m_input.group(1).strip('_'))

				m_latch = re.match( r'(.+)\s:\s(.+);', line.strip() )
				if prefix == 'l' and m_latch:
					var.append(m_latch.group(1).strip('_'))
					result += '''\n    %s = Bool('%s')'''%(m_latch.group(1).strip('_'), m_latch.group(1).strip('_'))
					result += '''\n    %s = Bool(str(%s) + '\\\'')'''%(m_latch.group(1).strip('_')+'p', m_latch.group(1).strip('_'))

				m_assign = re.match( r'(.+)\((.+)\)\s:=\s(.+);', line.strip() )
				if prefix == 'A' and m_assign:
					if m_assign.group(1) == "init":
						init_map[m_assign.group(2).strip('_')] = m_assign.group(3).strip('_')
					else:
						if '!' in m_assign.group(3):
							trans_map[m_assign.group(2).strip('_')] = 'Not(%s)'%m_assign.group(3).replace('!', '', 1).strip('_')
						elif m_assign.group(3) == 'TRUE':
							 trans_map[m_assign.group(2).strip('_')] = 'True'
						elif m_assign.group(3) == 'FALSE':
							 trans_map[m_assign.group(2).strip('_')] = 'False'
						else:
							trans_map[m_assign.group(2).strip('_')] = m_assign.group(3).strip('_')

				m_ands = re.match( r'(.+)\s:=\s(.+)\s&\s(.+);', line.strip() )
				if prefix == 'a' and m_ands:
					result += '''\n    %s = And(%s, %s)'''%(m_ands.group(1), 
															'Not(%s)'%m_ands.group(2).replace('!', "", 1).strip('_') if '!' in m_ands.group(2) else m_ands.group(2).strip('_'),
															'Not(%s)'%m_ands.group(3).replace('!', "", 1).strip('_') if '!' in m_ands.group(3) else m_ands.group(3).strip('_')
															)
				m_outputs = re.match( r'(.+)\s:=\s(.+);', line.strip())
				if prefix == 'o' and m_outputs:
					if '!' in m_outputs.group(2):
						prop = '%s'%m_outputs.group(2).replace('!', '', 1).strip('_')
					else:
						prop = 'Not(%s)'%m_outputs.group(2).replace('!', '', 1).strip('_')
