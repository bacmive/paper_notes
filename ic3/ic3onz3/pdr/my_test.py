...
def ExampleTwo():
	...
	variables = [Bool(str(i)) for i in range(len)]
	primes = [Bool(str(i) + '\'') for i in variables]
	init = And(*[Not(variables[i]) for i in range(len)])
	trans = And(*[
			Or(variables[0], Not(primes[1])),
			Or(Not(variables[0]), primes[1]),
			Or(variables[1], Not(primes[2])),
			Or(Not(variables[1]), primes[2])
		])
	post = Not(And(*[variables[i] for i in range(len)]))
	return (variables, primes, init, trans, post)
...
'ThreeAtATimeOdd':ThreeAtATimeOdd, 'ExampleOne':ExampleOne, 'ExampleTwo':ExampleTwo}
