...
class PDR(object):
    def __init__(self, literals, primes, init, trans, post):
        self.init = init  # And(x, y, Not(z))
        self.trans = trans  # And(Or...)
        self.literals = literals  #  variables == [x,y,z]
        self.lMap = {str(l):l for l in self.literals}  # lMap == {'y': y, 'x': x, 'z': z}
        self.post = post  # Or(Not(x), Not(y), Not(z))
        self.R = []  # 
        self.primeMap = zip(literals, primes)  # primeMap == [(x, xp), (y, yp), (z, zp)]

    def run(self):
        self.R = list()
        self.R.append(self.init)  # F0
    ...
    def checkForInduction(self):
        for frame in self.R:
            ...
            s.add(self.trans)
            s.add(frame)
            s.add(Not(substitute(frame, self.primeMap)))  # And(x, y, Not(z)) ==> And(xp, yp, Not(zp))
            if s.check() == unsat:  #  Fi & T & Not(Fi')  <==>  Not( Fi & T -> Fi' )
                return frame
        return None
    ...
    def getBadCube(self):
        model = And(Not(self.post), self.R[-1])  # self.post: Or(Not(x), Not(y), Not(z)); R0: init, R1: True
        s = Solver()
        s.add (model)
        if(s.check() == sat):
